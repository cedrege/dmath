...
const transposeTable = (tbody, newContainerType = "tbody") => {
    ...
    newTbody.setAttribute("border", 1);
    newTbody.setAttribute("id", "dijkstraSteps");
    ...
    tbody.parentElement.insertBefore(newTbody, tbody);
    ...
    switch (workMode) {
        ...
        case "setCostLabel":
            ...
            if(clickTarget.nodeName == "line") {
                ...
                //document.getElementById("divEdgeCost").children[0].children[2].focus();
                //document.getElementById("divEdgeCost").children[0].children[2].focus();
            }
            break;     
...
function drawVertex(px, py){
    ...
    vertex.label =  `L(${String.fromCharCode(nextLabelCode++)})`;
...
function dijkstra(source) {
    ...
    for(i=0; i< vertices.length; i++){
        all_labels.push(vertices[i].label.substring(2,3));
    }
    ...
    markedVertices =[];
    source.cost = 0;
    //hans.push(source.label.substring(2,3));

    //interface begin
    tr++;
    row=table.insertRow(tr);
    cell=row.insertCell(0);
    cell.innerHTML = round; //source.label.substring(2,3);
    for(i=0; i< vertices.length; i++){
        cell=row.insertCell(i+1);
    ...
    do {
        ...
        if(markedVertices.length < vertices.length) {
            ...
            cell=row.insertCell(0);
            cell.innerHTML = round;
            //cell.innerHTML = markedVertices[markedVertices.length - 1].label.substring(2,3);
            hans.add(markedVertices[markedVertices.length - 1].label.substring(2,3));
            for(i=0; i< vertices.length; i++){
                cell=row.insertCell(i+1);
                if(vertices[i].markedRound > round){
                    cell.innerHTML=(vertices[i].cost==INF)?'∞':vertices[i].cost;
                    cell.innerHTML+=', '+ ((vertices[i].previous==null)?'-':vertices[i].previous.label.substring(2,3));
                }
            }
        }
    ...
    // add Vertex row
    row_a=table.insertRow(tr);
    cell_a=row_a.insertCell(0);
    cell_a.innerHTML = "";
    var setIter = hans.entries();
    
    val = setIter.next();
    while(!val.done)
    {
        //console.log(val);
        cell_a=row_a.insertCell(-1);
        cell_a.innerHTML = val.value[0];
        val = setIter.next();
    }
